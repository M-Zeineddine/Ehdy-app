// TODO: Replace with a custom Congrats layout (stars, confetti burst, trophy, etc.)
import React from 'react';
import { BaseCard, type CardContentProps } from './BaseCard';
import { GIFT_THEMES } from '@/src/constants/giftThemes';

const theme = GIFT_THEMES.find(t => t.id === 'congrats')!;

interface Props extends CardContentProps {
  cardWidth: number;
  cardHeight: number;
}

export function CongratsCard(props: Props) {
  return <BaseCard {...props} theme={theme} />;
}
